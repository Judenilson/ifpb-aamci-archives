...
uint8_t handshake(void);  // função para realizar o handshake e testar conexão serial
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (!(GPIOA->IDR & 1)) // verifica se há início de comunicação !(GPIOA->IDR & 1) == !(...aaaaaaaa & ...00000001) == !(...0000000a) == ...0000000ã, se GPIOA->IDR == ...aaaaaaa0
    {
      ...
      if (recebido == 0b001) // recebeu um handshake
      {
        envia_cmd(0b001); // envia o mesmo handshake como ack
      }
    }
  }
}
...
uint8_t handshake(void)
{
  ...
  while (time > 0)
  {
    if (!(GPIOA->IDR & 1)) // verifica se há início de comunicação !(GPIOA->IDR & 1) == !(...aaaaaaaa & ...00000001) == !(...0000000a) == ...0000000ã, se GPIOA->IDR == ...aaaaaaa0
    {
      ...
      {
        return 1; // recebeu ack do handshake a tempo
      }
      else
        return 0; // recebeu retorno inesperado do handshake
    }
    else
      time--;
  }
  return 0; // não recebeu ack do handshake a tempo
}
